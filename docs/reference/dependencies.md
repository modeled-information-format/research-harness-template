...
detail see [the pack catalog](../packs/); for the tools each script calls
see [scripts](../scripts/).
...
| `mif-rh-cli` | `0.7.0+` — *repo-pinned in `scripts/fetch-engine.sh`* | Ontology classification (`resolve-ontology.sh`, `ontology-review.sh` delegate to it, ADR-0016) plus the suggest/calibrate loop — see [../engine-cli/](../engine-cli/) for the full subcommand surface and [../mcp-server/](../mcp-server/) for its MCP server | `mif-rh-cli --version` |
...
[How to instantiate the harness](../../how-to/instantiate-the-harness/).
...
[How to verify a release](../../how-to/verify-a-release/) and the policy in
