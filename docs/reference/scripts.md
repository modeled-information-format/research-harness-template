...
modified: '2026-07-13T17:26:54.309Z'
...
| `scripts/lib/container-lock.sh` | Sourced library, not a standalone script (issue #375/#382). Shared `mkdir`-based mutual-exclusion lock for `reports/<topic>/.container.lock`, used by both `mif-container-export.sh` and `mif-container-import.sh` so a concurrent export/import against the same topic fails closed instead of racing. Mirrors `run-lock.sh`'s staleness/steal semantics: a lock older than `CONTAINER_LOCK_STALE_MIN` (default 240min) is treated as abandoned and safely stolen via a race-safe rm-then-re-mkdir pattern. | none (pure shell) |
...
| `scripts/monitoring/lib/cron_match.py` | Minimal stdlib-only 5-field cron matcher (research-harness-template#424): since `monitor.yml`'s own GitHub Actions schedule trigger fires more often than any single topic's configured cron, each topic independently checks via this script whether the current firing is actually its turn. Exit 0 = matches, 1 = skip. | Python stdlib only |
