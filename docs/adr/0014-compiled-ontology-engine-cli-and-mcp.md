...
| PDD-2 (verify.sh / run-evals.sh unchanged) | `scripts/verify.sh`, `evals/run-evals.sh` temporarily re-pointed at `mif-rh-cli` via uncommitted wrapper scripts (measurement only — this repo's `scripts/verify.sh`/`run-evals.sh` are unchanged, see Summary) | compliant — 169/169 assertions passing under the compiled engine matched the bash baseline exactly (1 pre-existing, unrelated topic-README-staleness failure present in both); 42/42 evals passing, matching baseline. Both the finding/topic count (4296→4326, this ADR's own corpus growing over the days since drafting) and the assertion/eval counts (144→169, 41→42, `verify.sh`/`run-evals.sh` growing from unrelated merged work in the same window) are higher than this ADR's originally stated figures — the acceptance criterion actually measured is "the current suite's outcomes are identical whether run against bash or the compiled engine," not a literal match to the historical 144/41/4296 numbers, which are now stale by construction |
