...
gate_ontology_lock() {
  ...
  # Only in the template repo itself (copier.yml tracked, same instance-vs-
  # template signal copier-update.sh's own guard uses): ontologies.lock.json
  # must never be git-tracked here. A committed lock freezes the registry's
  ...
  if git -C . ls-files --error-unmatch copier.yml >/dev/null 2>&1; then
    if git -C . ls-files --error-unmatch ontologies.lock.json >/dev/null 2>&1; then
