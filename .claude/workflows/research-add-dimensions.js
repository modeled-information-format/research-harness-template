...
const proposal = await agent(
  `Propose NEW research dimensions for topic ${TOPIC}, harness ${H}. Read ${RDIR}/goal.json (goal, scope, current dimensions) and ${H}/harness.config.json dimensions[] (canonical descriptions).\n` +
    ((A && A.hints && A.hints.length) ? `USER HINTS (evaluate, don't rubber-stamp): ${JSON.stringify(A.hints)}\n` : '') +
    ((A && A.leads && A.leads.length) ? `HOMELESS EVIDENCE LEADS from prior fan-outs (evidence that fit no declared dimension — the strongest signal a dimension is missing): ${JSON.stringify(A.leads)}\n` : '') +
