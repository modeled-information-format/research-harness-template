    ...
    agent(
      `You are a research analyst for exactly ONE dimension of a goal-driven research session.\n` +
        ...
        (DEPTH === 'deep'
          ...
          : `cover the dimension's principal sub-areas. As a calibration reference, a well-scoped dimension is typically covered in single-digit-to-low-teens searches — treat that as a checkpoint, not a hard stop. Before stopping, confirm: (a) the last 2-3 searches are still turning up genuinely new germane material, not repeats — if not, stop; (b) every sub-area named in SCOPE above has at least one finding — if any is uncovered, keep going on that gap specifically; (c) no source you found implied a larger set exists than what you've captured (a registry/index/taxonomy naming more than you found) — if one did, keep going. State in saturationNote which of these, if any, justified continuing past the reference point, or confirm none did.`) +
