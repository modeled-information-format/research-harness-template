...
export const meta = {
  ...
  phases: [
    ...
    { title: 'Render', detail: 'mechanism 1: synthesize-artifact.sh -> render-artifact.sh per genre×channel row, synthesis cross-checked, then Skill(mif-docs:<genre>) applied to the body when genre!=="general" (#640/#744; no separate re-check needed, Route already confirmed pack enablement), then Skill(mif-docs:mif-provenance) witnessed-provenance stamp (ADR-0018, #632; declines gracefully when capture is off); mechanism 2: Skill(<channel>:<skillName>) invoked directly against findings (skillName differs from the channel name for diataxis -> diataxis-docs), synthesis never consulted, no genre axis EXCEPT ai-spec (#734, --genre passed through), provenance stamp explicitly not-applicable (each pack owns its own output/format)', model: 'sonnet' },
...
const RENDER_SCHEMA = {
  ...
  properties: {
    ...
    genreApplied: { type: 'boolean', description: '(#640, #734) true only if the Render step actually applied a genre — either mechanism 1\'s Skill(mif-docs:<genre>) body restructuring, or mechanism 2\'s "ai-spec" channel passing --genre through to its own skill; false when genre="general" (mechanism 1) or for every other mechanism-2 row (no genre axis) — never true for a genre whose template was not actually applied' },
    genreSkillInvoked: { type: 'string', description: '(#640/#744) the "mif-docs:<genre>" Skill reference actually invoked, or "" when genreApplied is false' },
...
// reference (genreSkillRef = `mif-docs:${genre}` — research-harness-template#744). Pack names are
...
const rendered = await pipeline(
  ...
  (p) => {
    ...
    const genreStepText =
      genreArg !== 'general'
        ? `4. GENRE SKILL APPLICATION (research-harness-template#640/#744): genre "${genreArg}"'s pack was already confirmed enabled by the ` +
          `Route phase (templateSource: ${p.templateSource}) — invoke Skill(mif-docs:${genreArg}) (every genre pack sources from the ` +
          `single "mif-docs" plugin, never a standalone "<genre>" plugin — research-harness-template#744) to restructure ONLY ` +
