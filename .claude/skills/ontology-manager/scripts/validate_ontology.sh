...
# Harness-grounded: default the schema to the vendored MIF ontology contract so the
# skill validates against the repo's authoritative copy without the caller passing it.
ROOT="$(cd "$(dirname "$0")/../../../.." && pwd)"
FILE="${1:?Usage: validate_ontology.sh <file.yaml> [schema.json]}"
SCHEMA="${2:-$ROOT/schemas/mif/ontology.schema.json}"
ERRORS=0
WARNINGS=0
...
if [[ -n "$SCHEMA" && -f "$SCHEMA" ]]; then
	...
	# Harness validator: ajv (the repo's JSON Schema tool; no PyPI). draft2020 +
	# ajv-formats so the schema's uri formats resolve.
	if command -v ajv >/dev/null 2>&1; then
		if ajv validate --spec=draft2020 --strict=false -c ajv-formats -s "$SCHEMA" -d "$TMPJSON" >/dev/null 2>&1; then
			ok "JSON Schema validation passed (ajv)"
		else
			err "JSON Schema validation failed (ajv)"
		fi
	else
		warn "ajv not found — skipping JSON Schema validation"
	fi
fi
