---
slug: reports/example-okf-mif-knowledge-spine/report-computing-paper
version: 1
...
```mermaid
flowchart TB
    subgraph OKF["OKF v0.1 packaging surface (low authoring cost)"]
      A["Markdown body (human and LLM readable)"]
      B["YAML frontmatter: type, title, tags, timestamp"]
      C["Untyped markdown links"]
      D["log.md plus Citations prose"]
    end
    subgraph MIF["MIF v1.0.0 semantic layer (frontmatter extensions)"]
      E["ontology plus entity block"]
      F["relationships: 9 typed predicates plus strength"]
      G["provenance: W3C PROV-O compatible JSON"]
      H["bi-temporal plus decay: TTL, half-life, models"]
    end
    B -->|permissive extension seam| E
    B --> F
    B --> G
    B --> H
    C -.->|upgraded to| F
    D -.->|replaced by| G
    MIF -->|fail-closed validation of MIF layer| OKF
```
