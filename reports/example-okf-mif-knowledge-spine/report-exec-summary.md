---
slug: reports/example-okf-mif-knowledge-spine/report-exec-summary
version: 1
'@context': https://mif-spec.dev/schema/context.jsonld
'@type': Concept
'@id': urn:mif:report:harness/example-okf-mif-knowledge-spine:report-exec-summary
conceptType: semantic
namespace: harness/example-okf-mif-knowledge-spine
title: 'Executive Summary: Should MIF Be the Spine Beneath OKF for a Research Knowledge Spine?'
created: "2026-06-28T14:09:45Z"
modified: "2026-06-28T14:09:45Z"
temporal:
  "@type": TemporalMetadata
  validFrom: "2026-06-28T14:09:45Z"
  ttl: P6M
  recordedAt: "2026-06-28T14:09:45Z"
provenance:
  '@type': Provenance
  sourceType: system_generated
  confidence: 0.9
  trustLevel: moderate_confidence
citations:
  - '@type': Citation
    citationType: website
    citationRole: refutes
    title: a16z 'How 100 Enterprise CIOs Are Building and Buying Gen AI in 2025' - source does not substantiate the specific 76%/50-50 build-vs-buy figure on inspection (only a qualitative shift-to-buying)
    url: https://a16z.com/ai-enterprise-2025/
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'JSON-LD Schema Markup for AI Discoverability: Technical Guide 2026 - AgentVisibility.ai'
    url: https://agentvisibility.ai/insights/json-ld-schema-ai-discoverability
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'Governing Evolving Memory in LLM Agents: Risks, Mechanisms, and the SSGM Framework — arXiv'
    url: https://arxiv.org/html/2603.11768v1
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: A Decade of Scholarly Research on Open Knowledge Graphs - Research community KG adoption (arXiv)
    url: https://arxiv.org/pdf/2306.13186
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: OWL Reasoners still useable in 2023 (arXiv)
    url: https://arxiv.org/pdf/2309.06888
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'Semantic Web: Past, Present, and Future — arXiv 2412.17159'
    url: https://arxiv.org/pdf/2412.17159
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: Semantic Web and Software Agents — A Forgotten Wave of Artificial Intelligence? arXiv 2503.20793
    url: https://arxiv.org/pdf/2503.20793
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'PROV-AGENT: Unified Provenance for Tracking AI Agent Interactions in Agentic Workflows (arXiv)'
    url: https://arxiv.org/pdf/2508.02866
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'Gartner on Context Graphs: Trends, Capabilities, Setup in 2026 — Atlan'
    url: https://atlan.com/know/gartner-context-graphs/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: 'Ontology vs. Semantic Layer: Differences and schema.org limitations — Atlan'
    url: https://atlan.com/know/ontology-vs-semantic-layer/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: 'RDF vs OWL: Key Differences, Use Cases and Examples Explained - Atlan'
    url: https://atlan.com/know/rdf-vs-owl/
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: Stardog Enterprise Knowledge Graph Platform Pricing (AWS Marketplace)
    url: https://aws.amazon.com/marketplace/pp/prodview-ulfm6fel7xgjq
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: Frictionless Data and FAIR Research Principles - Open Knowledge Foundation Blog
    url: https://blog.okfn.org/2018/08/14/frictionless-data-and-fair-research-principles/
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: Knowledge Management Statistics and Trends in 2025 - Worker productivity costs (CAKE)
    url: https://cake.com/blog/knowledge-management-statistics/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: How the Open Knowledge Format can improve data sharing — Google Cloud Blog
    url: https://cloud.google.com/blog/products/data-analytics/how-the-open-knowledge-format-can-improve-data-sharing
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'Ontologies, Context Graphs, and Semantic Layers: What AI Actually Needs in 2026'
    url: https://contextandchaos.substack.com/p/ontologies-context-graphs-and-semantic
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: Knowledge Management and Dissemination for Think Tanks (DataCalculus)
    url: https://datacalculus.com/en/blog/think-tanks/program-director/knowledge-management-and-dissemination-for-think-tanks
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: Personal Knowledge Management Software Market Research Report 2034 — DataIntelo
    url: https://dataintelo.com/report/personal-knowledge-management-software-market
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: Lessons Learned from the Combined Development of OWL and SHACL — ACM K-CAP 2025
    url: https://dl.acm.org/doi/full/10.1145/3731443.3771340
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: Top Knowledge Management Trends 2026 - Semantic layers and enterprise AI (Enterprise Knowledge)
    url: https://enterprise-knowledge.com/top-knowledge-management-trends-2026/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: LLM Wiki — Karpathy GitHub Gist (April 2026)
    url: https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: OKF SPEC.md — GoogleCloudPlatform/knowledge-catalog
    url: https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: Frictionless Data Package — GitHub frictionlessdata/datapackage
    url: https://github.com/frictionlessdata/datapackage
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: MIF v1.0 — GitHub zircote/MIF
    url: https://github.com/zircote/MIF
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: Open Knowledge Format (OKF) — Official Grounding Page
    url: https://groundingpage.com/facts/open-knowledge-format/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: JSON-LD - JSON for Linked Data (Official Site)
    url: https://json-ld.org/
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: Google Cloud Launches Open Knowledge Format Standard - sober adoption assessment (Let's Data Science)
    url: https://letsdatascience.com/news/google-cloud-launches-open-knowledge-format-standard-b9480a66
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'From LLMs to Knowledge Graphs: Building Production-Ready Graph Systems in 2025 — Medium'
    url: https://medium.com/@claudiubranzan/from-llms-to-knowledge-graphs-building-production-ready-graph-systems-in-2025-2b4aff1ec99a
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'Beyond OWL: Reconsidering Ontologies in the Age of AI and the Semantic Web'
    url: https://medium.com/@nfigay/beyond-owl-reconsidering-ontologies-in-the-age-of-ai-and-the-semantic-web-4059b519f23d
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: Open-Sourcing the Knowledge Graph Studio under MIT license (Medium/Enterprise RAG)
    url: https://medium.com/enterprise-rag/open-sourcing-the-whyhow-knowledge-graph-studio-powered-by-nosql-edce283fb341
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'State of AI Agent Memory 2026: Benchmarks, Architectures & Production Gaps — Mem0'
    url: https://mem0.ai/blog/state-of-ai-agent-memory-2026
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: MIF Schema Reference — mif-spec.dev
    url: https://mif-spec.dev/
  - '@type': Citation
    citationType: website
    citationRole: refutes
    title: MIF relationship types (mif-spec.dev) - the core vocabulary is relates-to/derived-from/supersedes/conflicts-with/part-of/implements/uses/created-by/mentioned-in; supports/contradicts/refines/depends-on/updates are not MIF-native core, only custom namespaced
    url: https://mif-spec.dev/specification/relationship-types/
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: 'Open-Source vs SaaS Agent Platforms: Pros & Cons for Enterprises (OneReach.ai)'
    url: https://onereach.ai/blog/open-source-frameworks-vs-saas-agent-platforms/
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: Enterprise Knowledge Graph Buyer's Guide 2026 - Pricing and ROI signals (Promethium)
    url: https://promethium.ai/guides/enterprise-knowledge-graph-buyers-guide-2026/
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'Graph RAG Guide 2025: Architecture, Implementation & ROI — Salfati Group'
    url: https://salfati.group/topics/graph-rag
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'Obsidian Complete Guide: The Ultimate Markdown Editor for Knowledge Management Revolution 2025 — SmartScope'
    url: https://smartscope.blog/en/obsidian-complete-guide/
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'Obsidian vs Logseq 2026: Which PKM Tool Wins? - SoftPicker'
    url: https://softpicker.com/obsidian-vs-logseq/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: Frictionless Data Specifications - Official Home
    url: https://specs.frictionlessdata.io/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: Frictionless Data Package Specification — specs.frictionlessdata.io
    url: https://specs.frictionlessdata.io/data-package/
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: State of Open Data 2025 - FAIR data and open science trends
    url: https://stateofopendata.com/
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: Knowledge Management Software Market Size, Share, Growth, 2034 (Straits Research)
    url: https://straitsresearch.com/report/knowledge-management-software-market
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: 'AI Hallucination Statistics 2026: 50+ Sourced Data Points (Suprmind)'
    url: https://suprmind.ai/hub/insights/ai-hallucination-statistics-research-report-2026/
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: Bi-temporal memory for AI coding agents — git-pinned context that survives context compaction
    url: https://sverklo.com/blog/bi-temporal-memory-for-ai-agents/
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: Google Launches a Universal Format for Karpathy's LLM Wiki — Techstrong.ai
    url: https://techstrong.ai/articles/google-launches-a-universal-format-for-karpathys-llm-wiki/
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: Google Just Standardized Karpathy's LLM Wiki Pattern — The Menon Lab
    url: https://themenonlab.blog/blog/google-okf-open-knowledge-format-karpathy-llm-wiki-standard
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: 'Obsidian Pricing 2026: Plans, Hidden Costs & Cheaper Alternatives (ToolRadar)'
    url: https://toolradar.com/tools/obsidian/pricing
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'Agent-to-agent audit trail: provenance for AI ecosystems (TrueScreen)'
    url: https://truescreen.io/articles/agent-to-agent-audit-trail/
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: Personal Knowledge Graphs in Obsidian - Volodymyr Pavlyshyn, Medium
    url: https://volodymyrpavlyshyn.medium.com/personal-knowledge-graphs-in-obsidian-528a0f4584b9
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: Why Bad Knowledge Management Is Killing Your Profits (WikiTeq)
    url: https://wikiteq.com/post/hidden-costs-poor-knowledge-management
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: '2026 Enterprise AI Knowledge Management: AI-native KM market size (Windows Forum/GoSearch)'
    url: https://windowsforum.com/threads/2026-enterprise-ai-knowledge-management-from-search-to-governed-agent-workflows.410816/
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: Open Knowledge Format (OKF) Complete 2026 Guide - ecosystem gaps identified (WitsCode)
    url: https://witscode.com/open-knowledge-format
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: AI-Ready Enterprise Knowledge Graph Market to Reach USD 6,550.0 Million by 2036 (AccessNewswire/FMI)
    url: https://www.accessnewswire.com/newsroom/en/business-and-professional-services/ai-ready-enterprise-knowledge-graph-market-to-reach-usd-6-550.0-1167718
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: Knowledge Management Software Market Size, Industry Share | Forecast 2034 (Fortune Business Insights)
    url: https://www.fortunebusinessinsights.com/knowledge-management-software-market-110376
  - '@type': Citation
    citationType: website
    citationRole: refutes
    title: Gartner Predicts 40% of Enterprise Apps Will Feature Task-Specific AI Agents by 2026, Up from Less Than 5% in 2025 (Gartner Newsroom)
    url: https://www.gartner.com/en/newsroom/press-releases/2025-08-26-gartner-predicts-40-percent-of-enterprise-apps-will-feature-task-specific-ai-agents-by-2026-up-from-less-than-5-percent-in-2025
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: Enterprise Knowledge Graph Market Industry Report 2033 — Grand View Research
    url: https://www.grandviewresearch.com/industry-analysis/enterprise-knowledge-graph-market-report
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: The Cost and Consequence of Institutional Memory Drain (Inc. Magazine)
    url: https://www.inc.com/bethmaser/the-cost-and-consequence-of-institutional-memory-drain/91178504
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: Simple Knowledge Organization System (SKOS) — ISKO Encyclopedia of KO
    url: https://www.isko.org/cyclo/skos.htm
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: Cost of Organizational Knowledge Loss and Countermeasures (Iterators HQ)
    url: https://www.iteratorshq.com/blog/cost-of-organizational-knowledge-loss-and-countermeasures/
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: Why AI Hallucinates in Your Enterprise (and how Context Graphs Fix it) - Kamiwaza
    url: https://www.kamiwaza.ai/insights/why-ai-hallucinates-in-your-enterprise
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: Knowledge Graph Market Worth $9.88 Billion by 2032 — MarketsandMarkets
    url: https://www.marketsandmarkets.com/PressReleases/knowledge-graph.asp
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: Google Cloud Introduces Open Knowledge Format (OKF) — MarkTechPost
    url: https://www.marktechpost.com/2026/06/16/google-cloud-introduces-open-knowledge-format-okf-a-vendor-neutral-markdown-spec-for-giving-ai-agents-curated-context/
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'Knowledge Graph vs Vector Database for RAG: Which Is Best? — Meilisearch'
    url: https://www.meilisearch.com/blog/knowledge-graph-vs-vector-database-for-rag
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'GraphRAG: Unlocking LLM Discovery on Narrative Private Data — Microsoft Research Blog'
    url: https://www.microsoft.com/en-us/research/blog/graphrag-unlocking-llm-discovery-on-narrative-private-data/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: Project GraphRAG — Microsoft Research
    url: https://www.microsoft.com/en-us/research/project/graphrag/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: A Semantic Approach to Mapping the Provenance Ontology to Basic Formal Ontology — Scientific Data
    url: https://www.nature.com/articles/s41597-025-04580-1
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: Notion vs Obsidian - minimalism as user preference (NotionApps)
    url: https://www.notionapps.com/blog/notion-vs-obsidian-knowledge-productivity-2025
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'The Semantic Web: 20 Years and a Handful of Enterprise Knowledge Graphs Later — Ontotext'
    url: https://www.ontotext.com/blog/the-semantic-web-20-years-later/
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: 'Notion vs Obsidian vs Roam Research 2025: Best Note-Taking App for Productivity'
    url: https://www.primeproductiv4.com/blog-articles/notion-vs-obsidian-vs-roam-research-productivity-comparison
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'History of Obsidian: Second Brain to AI Knowledge OS — Taskade Blog'
    url: https://www.taskade.com/blog/obsidian-history
  - '@type': Citation
    citationType: website
    citationRole: refutes
    title: AI-Driven Knowledge Management System Market Report (The Business Research Company) - the '$7.71B 2025 / 47.2%' figure traces here, not to GoSearch; cross-firm AI-KM sizing varies widely and the finding's two growth rates do not reconcile
    url: https://www.thebusinessresearchcompany.com/report/ai-driven-knowledge-management-system-global-market-report
  - '@type': Citation
    citationType: website
    citationRole: supports
    title: Neo4j Software Pricing & Plans 2026 (Vendr)
    url: https://www.vendr.com/marketplace/neo4j
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: SKOS Simple Knowledge Organization System - W3C Home Page
    url: https://www.w3.org/2004/02/skos/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: RDF & SPARQL Working Group Charter — W3C (April 2025)
    url: https://www.w3.org/2025/04/rdf-star-wg-charter.html
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: JSON-LD 1.1 — W3C Recommendation
    url: https://www.w3.org/TR/json-ld11/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: 'PROV-O: The PROV Ontology - W3C Recommendation'
    url: https://www.w3.org/TR/prov-o/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: PROV-Overview — W3C
    url: https://www.w3.org/TR/prov-overview/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: SKOS Simple Knowledge Organization System Primer - W3C Recommendation
    url: https://www.w3.org/TR/skos-primer/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: SKOS Simple Knowledge Organization System Reference — W3C
    url: https://www.w3.org/TR/skos-reference/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: Ontologies and Knowledge Graphs in Industry Community Group — W3C
    url: https://www.w3.org/community/oki/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: YAML-LD — W3C CG Final Report, December 2023
    url: https://www.w3.org/community/reports/json-ld/CG-FINAL-yaml-ld-20231206/
  - '@type': Citation
    citationType: documentation
    citationRole: supports
    title: The PROV-JSONLD Serialization - W3C Member Submission 2024
    url: https://www.w3.org/submissions/2024/SUBM-prov-jsonld-20240825/
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'Introducing MIF: Memory Interchange Format — zircote.com (February 2026)'
    url: https://zircote.com/blog/2026/02/introducing-mif-memory-interchange-format/
  - '@type': Citation
    citationType: article
    citationRole: supports
    title: 'AI Agent Memory Architectures: From Context Windows to Persistent Knowledge — Zylos Research'
    url: https://zylos.ai/research/2026-04-05-ai-agent-memory-architectures-persistent-knowledge/
extensions:
  harness:
    dimension: synthesis
    verification:
      verdict: survived
      verdict_basis: 'Report-level verdict aggregating the corpus adversarial falsification gate (2026-06-28; see 2026-06-28-falsification-report.md): all 36 supporting findings were adversarially tested exactly once — 31 survived, 5 weakened, 0 falsified, 0 inconclusive. The 5 weakened findings (4 market statistics + 1 technical enumeration) travel with explicit caveats; no finding underpinning the central OKF+MIF layering thesis was falsified. The MIF-version sub-claim was additionally corrected against the authoritative in-repo SPECIFICATION.md (v1.0.0, Released, stabilized 2026-06-18; public since ~Feb 2026), which supersedes the analyst''s stale web sources. The report thesis therefore inherits a surviving verdict with documented weakened-statistic caveats. This genre report is a projection of that single graded synthesis.'
      disconfirming_evidence: []
      attempted_at: "2026-06-28T12:25:00Z"
---

This exec-summary synthesis covers 36 surviving finding(s) across the research.

## BLUF (Bottom Line Up Front)

Yes — layer MIF as the modeling, provenance, and temporal spine beneath OKF as the git-distributable packaging surface. The research clears all four decision bars: the layering is technically feasible, MIF is differentiated from existing knowledge-representation prior art, the adoption trajectory favors structured and provenance-backed knowledge, and documented buyer segments carry the pains the spine addresses. Act now — build a thin reference implementation that emits OKF-valid bundles carrying MIF frontmatter extensions, and validate it against one provenance-heavy workflow before funding a productized governance layer. The decision turns on a single open risk: Google Cloud shipped OKF only weeks ago (12 June 2026) with no ecosystem, and its very minimalism may be what buyers want — so the pilot must prove buyers will pay for MIF's richer layer, not merely adopt OKF's simpler one.

## Key Findings

Feasibility: OKF v0.1 lets producers add any frontmatter keys and requires consumers to preserve unknown keys, so MIF's typed relationships, provenance, temporal, and ontology fields inject cleanly as OKF frontmatter extensions. The one real tension is OKF's permissive-consumer model against MIF's fail-closed validation — a design choice to resolve, not a blocker.

Differentiation: OKF v0.1 types every inter-concept link only in surrounding prose and records a single last-changed timestamp. MIF adds typed, directed relationship edges with optional strength (0-1), a first-class W3C PROV-O-compatible provenance object (source type, confidence, trust level, attribution chain), and bi-temporal tracking with configurable decay — replacing OKF's informal conventions with a machine-verifiable record.

Trajectory: Hybrid graph-plus-vector retrieval now beats pure vector RAG by 3.4x on accuracy and clears 90%+ on schema-bound queries [3]; the enterprise knowledge-graph market compounds at 21-36% annually, with Gartner projecting that more than half of AI-agent systems will run on context graphs by 2028 [4]; and 2026 agent-memory research names staleness and provenance as the two hardest unsolved problems — the precise gap MIF closes [5].

Prior art: The Semantic Web stalled by prioritizing logical completeness (RDF, OWL, SPARQL) over usability while minimal Schema.org won — and an OKF+MIF spine deliberately follows that pragmatic path. RDF/OWL, JSON-LD, SKOS, and Frictionless Data each cover only part of the spine (formal ontology, linked data, taxonomy, or dataset packaging); none combine git-distributable packaging with citation-backed findings, typed edges, and bi-temporal provenance.

Market: AI/ML, knowledge-engineering, research, think-tank, and developer-platform teams each carry documented provenance, LLM-grounding, or institutional-memory pain that the spine addresses. The demand direction is real, but several supporting market statistics are weakened (see Risks & Caveats) and should be read as directional, not load-bearing.

## Recommendation

> Build a thin OKF+MIF reference implementation and validate it against one provenance-heavy research workflow before funding a commercial governance layer.

What: Ship a producer/consumer pair that emits OKF-valid v0.1 bundles whose frontmatter carries MIF extensions — typed relationships, the provenance object, and bi-temporal fields — plus a validator that enforces MIF's fail-closed rules over OKF's permissive base.

Why: Every technical and differentiation bar passes and the demand trajectory is favorable, so the binding constraint is not feasibility. It is proving a buyer will pay for the richer layer rather than settle for OKF alone.

How: Pick one workflow where provenance and staleness already hurt — research synthesis or agent memory — instrument it end to end, and measure whether MIF's typed, provenance, and temporal layer changes outcomes a user will pay for.

Risk: If the pilot shows OKF's minimalism already satisfies the workflow, treat MIF as an optional enrichment profile rather than a productized layer, and defer any governance or hosting business until a second segment confirms willingness to pay.

## Risks & Caveats

OKF nascency is the dominant risk. Google Cloud published OKF v0.1 on 12 June 2026, only days before this research; the repository drew 5,440 stars and 416 forks quickly [1] but ships with no producer libraries, consumer integrations, governance, or enterprise adoption record. Demand for OKF specifically — as distinct from structured knowledge formats generally — remains undemonstrated, and OKF's minimalism may itself be the buyer preference, which would leave MIF's richer layer a solution ahead of its segment.

MIF's gap is distribution, not maturity. MIF has been public since February 2026 and reached v1.0.0 (Released, stabilized 18 June 2026) [2], so it predates OKF v0.1 and is the more stabilized specification. Its challenge is building an independent adopter base and formal governance against OKF's Google-backed launch — not closing a spec-maturity gap.

Five claims carry caveats (confidence basis). Four market statistics are weakened and travel as directional only: an enterprise AI-agent adoption figure (a cited "80% by Q1 2026" overstates Gartner's own roughly 40%-by-2026 / under-5%-in-2025 trajectory), a "76% SaaS, up from 50-50 in 2024" build-versus-buy split the cited source does not substantiate on inspection, and knowledge-graph pricing and growth bands (self-managed platforms at $15K-100K+/year, personal tools at $5-16/user/month, AI-driven knowledge management growing around 47% year over year). Separately, the gate narrowed one technical claim: MIF does provide typed, directed edges with optional strength that OKF lacks (core types include relates-to, derived-from, supersedes, and part-of), but the broader nine-type enumeration mixes in harness-specific custom types and must not be cited as MIF core. The GraphRAG accuracy, enterprise-KG growth, and agent-memory provenance-demand findings survived unweakened and carry the trajectory case.

The confidence basis: the adversarial falsification gate tested 36 findings exactly once; 31 survived and 5 were weakened with explicit caveats, and none was falsified.

```mermaid
pie showData title Corpus evidence base — 36 adversarially tested findings
    "Survived" : 31
    "Weakened (caveated)" : 5
```

## Footnotes

- [1] Google Cloud — How the Open Knowledge Format can improve data sharing: <https://cloud.google.com/blog/products/data-analytics/how-the-open-knowledge-format-can-improve-data-sharing>
- [2] MIF specification, v1.0.0 Released: <https://mif-spec.dev/>
- [3] Microsoft Research — GraphRAG: <https://www.microsoft.com/en-us/research/project/graphrag/>
- [4] Gartner on context graphs (via Atlan): <https://atlan.com/know/gartner-context-graphs/>
- [5] State of AI Agent Memory 2026 (mem0): <https://mem0.ai/blog/state-of-ai-agent-memory-2026>

## Sources

- [a16z 'How 100 Enterprise CIOs Are Building and Buying Gen AI in 2025' - source does not substantiate the specific 76%/50-50 build-vs-buy figure on inspection (only a qualitative shift-to-buying)](<https://a16z.com/ai-enterprise-2025/>)
- [JSON-LD Schema Markup for AI Discoverability: Technical Guide 2026 - AgentVisibility.ai](<https://agentvisibility.ai/insights/json-ld-schema-ai-discoverability>)
- [Governing Evolving Memory in LLM Agents: Risks, Mechanisms, and the SSGM Framework — arXiv](<https://arxiv.org/html/2603.11768v1>)
- [A Decade of Scholarly Research on Open Knowledge Graphs - Research community KG adoption (arXiv)](<https://arxiv.org/pdf/2306.13186>)
- [OWL Reasoners still useable in 2023 (arXiv)](<https://arxiv.org/pdf/2309.06888>)
- [Semantic Web: Past, Present, and Future — arXiv 2412.17159](<https://arxiv.org/pdf/2412.17159>)
- [Semantic Web and Software Agents — A Forgotten Wave of Artificial Intelligence? arXiv 2503.20793](<https://arxiv.org/pdf/2503.20793>)
- [PROV-AGENT: Unified Provenance for Tracking AI Agent Interactions in Agentic Workflows (arXiv)](<https://arxiv.org/pdf/2508.02866>)
- [Gartner on Context Graphs: Trends, Capabilities, Setup in 2026 — Atlan](<https://atlan.com/know/gartner-context-graphs/>)
- [Ontology vs. Semantic Layer: Differences and schema.org limitations — Atlan](<https://atlan.com/know/ontology-vs-semantic-layer/>)
- [RDF vs OWL: Key Differences, Use Cases and Examples Explained - Atlan](<https://atlan.com/know/rdf-vs-owl/>)
- [Stardog Enterprise Knowledge Graph Platform Pricing (AWS Marketplace)](<https://aws.amazon.com/marketplace/pp/prodview-ulfm6fel7xgjq>)
- [Frictionless Data and FAIR Research Principles - Open Knowledge Foundation Blog](<https://blog.okfn.org/2018/08/14/frictionless-data-and-fair-research-principles/>)
- [Knowledge Management Statistics and Trends in 2025 - Worker productivity costs (CAKE)](<https://cake.com/blog/knowledge-management-statistics/>)
- [How the Open Knowledge Format can improve data sharing — Google Cloud Blog](<https://cloud.google.com/blog/products/data-analytics/how-the-open-knowledge-format-can-improve-data-sharing>)
- [Ontologies, Context Graphs, and Semantic Layers: What AI Actually Needs in 2026](<https://contextandchaos.substack.com/p/ontologies-context-graphs-and-semantic>)
- [Knowledge Management and Dissemination for Think Tanks (DataCalculus)](<https://datacalculus.com/en/blog/think-tanks/program-director/knowledge-management-and-dissemination-for-think-tanks>)
- [Personal Knowledge Management Software Market Research Report 2034 — DataIntelo](<https://dataintelo.com/report/personal-knowledge-management-software-market>)
- [Lessons Learned from the Combined Development of OWL and SHACL — ACM K-CAP 2025](<https://dl.acm.org/doi/full/10.1145/3731443.3771340>)
- [Top Knowledge Management Trends 2026 - Semantic layers and enterprise AI (Enterprise Knowledge)](<https://enterprise-knowledge.com/top-knowledge-management-trends-2026/>)
- [LLM Wiki — Karpathy GitHub Gist (April 2026)](<https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f>)
- [OKF SPEC.md — GoogleCloudPlatform/knowledge-catalog](<https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md>)
- [Frictionless Data Package — GitHub frictionlessdata/datapackage](<https://github.com/frictionlessdata/datapackage>)
- [MIF v1.0 — GitHub zircote/MIF](<https://github.com/zircote/MIF>)
- [Open Knowledge Format (OKF) — Official Grounding Page](<https://groundingpage.com/facts/open-knowledge-format/>)
- [JSON-LD - JSON for Linked Data (Official Site)](<https://json-ld.org/>)
- [Google Cloud Launches Open Knowledge Format Standard - sober adoption assessment (Let's Data Science)](<https://letsdatascience.com/news/google-cloud-launches-open-knowledge-format-standard-b9480a66>)
- [From LLMs to Knowledge Graphs: Building Production-Ready Graph Systems in 2025 — Medium](<https://medium.com/@claudiubranzan/from-llms-to-knowledge-graphs-building-production-ready-graph-systems-in-2025-2b4aff1ec99a>)
- [Beyond OWL: Reconsidering Ontologies in the Age of AI and the Semantic Web](<https://medium.com/@nfigay/beyond-owl-reconsidering-ontologies-in-the-age-of-ai-and-the-semantic-web-4059b519f23d>)
- [Open-Sourcing the Knowledge Graph Studio under MIT license (Medium/Enterprise RAG)](<https://medium.com/enterprise-rag/open-sourcing-the-whyhow-knowledge-graph-studio-powered-by-nosql-edce283fb341>)
- [State of AI Agent Memory 2026: Benchmarks, Architectures & Production Gaps — Mem0](<https://mem0.ai/blog/state-of-ai-agent-memory-2026>)
- [MIF Schema Reference — mif-spec.dev](<https://mif-spec.dev/>)
- [MIF relationship types (mif-spec.dev) - the core vocabulary is relates-to/derived-from/supersedes/conflicts-with/part-of/implements/uses/created-by/mentioned-in; supports/contradicts/refines/depends-on/updates are not MIF-native core, only custom namespaced](<https://mif-spec.dev/specification/relationship-types/>)
- [Open-Source vs SaaS Agent Platforms: Pros & Cons for Enterprises (OneReach.ai)](<https://onereach.ai/blog/open-source-frameworks-vs-saas-agent-platforms/>)
- [Enterprise Knowledge Graph Buyer's Guide 2026 - Pricing and ROI signals (Promethium)](<https://promethium.ai/guides/enterprise-knowledge-graph-buyers-guide-2026/>)
- [Graph RAG Guide 2025: Architecture, Implementation & ROI — Salfati Group](<https://salfati.group/topics/graph-rag>)
- [Obsidian Complete Guide: The Ultimate Markdown Editor for Knowledge Management Revolution 2025 — SmartScope](<https://smartscope.blog/en/obsidian-complete-guide/>)
- [Obsidian vs Logseq 2026: Which PKM Tool Wins? - SoftPicker](<https://softpicker.com/obsidian-vs-logseq/>)
- [Frictionless Data Specifications - Official Home](<https://specs.frictionlessdata.io/>)
- [Frictionless Data Package Specification — specs.frictionlessdata.io](<https://specs.frictionlessdata.io/data-package/>)
- [State of Open Data 2025 - FAIR data and open science trends](<https://stateofopendata.com/>)
- [Knowledge Management Software Market Size, Share, Growth, 2034 (Straits Research)](<https://straitsresearch.com/report/knowledge-management-software-market>)
- [AI Hallucination Statistics 2026: 50+ Sourced Data Points (Suprmind)](<https://suprmind.ai/hub/insights/ai-hallucination-statistics-research-report-2026/>)
- [Bi-temporal memory for AI coding agents — git-pinned context that survives context compaction](<https://sverklo.com/blog/bi-temporal-memory-for-ai-agents/>)
- [Google Launches a Universal Format for Karpathy's LLM Wiki — Techstrong.ai](<https://techstrong.ai/articles/google-launches-a-universal-format-for-karpathys-llm-wiki/>)
- [Google Just Standardized Karpathy's LLM Wiki Pattern — The Menon Lab](<https://themenonlab.blog/blog/google-okf-open-knowledge-format-karpathy-llm-wiki-standard>)
- [Obsidian Pricing 2026: Plans, Hidden Costs & Cheaper Alternatives (ToolRadar)](<https://toolradar.com/tools/obsidian/pricing>)
- [Agent-to-agent audit trail: provenance for AI ecosystems (TrueScreen)](<https://truescreen.io/articles/agent-to-agent-audit-trail/>)
- [Personal Knowledge Graphs in Obsidian - Volodymyr Pavlyshyn, Medium](<https://volodymyrpavlyshyn.medium.com/personal-knowledge-graphs-in-obsidian-528a0f4584b9>)
- [Why Bad Knowledge Management Is Killing Your Profits (WikiTeq)](<https://wikiteq.com/post/hidden-costs-poor-knowledge-management>)
- [2026 Enterprise AI Knowledge Management: AI-native KM market size (Windows Forum/GoSearch)](<https://windowsforum.com/threads/2026-enterprise-ai-knowledge-management-from-search-to-governed-agent-workflows.410816/>)
- [Open Knowledge Format (OKF) Complete 2026 Guide - ecosystem gaps identified (WitsCode)](<https://witscode.com/open-knowledge-format>)
- [AI-Ready Enterprise Knowledge Graph Market to Reach USD 6,550.0 Million by 2036 (AccessNewswire/FMI)](<https://www.accessnewswire.com/newsroom/en/business-and-professional-services/ai-ready-enterprise-knowledge-graph-market-to-reach-usd-6-550.0-1167718>)
- [Knowledge Management Software Market Size, Industry Share | Forecast 2034 (Fortune Business Insights)](<https://www.fortunebusinessinsights.com/knowledge-management-software-market-110376>)
- [Gartner Predicts 40% of Enterprise Apps Will Feature Task-Specific AI Agents by 2026, Up from Less Than 5% in 2025 (Gartner Newsroom)](<https://www.gartner.com/en/newsroom/press-releases/2025-08-26-gartner-predicts-40-percent-of-enterprise-apps-will-feature-task-specific-ai-agents-by-2026-up-from-less-than-5-percent-in-2025>)
- [Enterprise Knowledge Graph Market Industry Report 2033 — Grand View Research](<https://www.grandviewresearch.com/industry-analysis/enterprise-knowledge-graph-market-report>)
- [The Cost and Consequence of Institutional Memory Drain (Inc. Magazine)](<https://www.inc.com/bethmaser/the-cost-and-consequence-of-institutional-memory-drain/91178504>)
- [Simple Knowledge Organization System (SKOS) — ISKO Encyclopedia of KO](<https://www.isko.org/cyclo/skos.htm>)
- [Cost of Organizational Knowledge Loss and Countermeasures (Iterators HQ)](<https://www.iteratorshq.com/blog/cost-of-organizational-knowledge-loss-and-countermeasures/>)
- [Why AI Hallucinates in Your Enterprise (and how Context Graphs Fix it) - Kamiwaza](<https://www.kamiwaza.ai/insights/why-ai-hallucinates-in-your-enterprise>)
- [Knowledge Graph Market Worth $9.88 Billion by 2032 — MarketsandMarkets](<https://www.marketsandmarkets.com/PressReleases/knowledge-graph.asp>)
- [Google Cloud Introduces Open Knowledge Format (OKF) — MarkTechPost](<https://www.marktechpost.com/2026/06/16/google-cloud-introduces-open-knowledge-format-okf-a-vendor-neutral-markdown-spec-for-giving-ai-agents-curated-context/>)
- [Knowledge Graph vs Vector Database for RAG: Which Is Best? — Meilisearch](<https://www.meilisearch.com/blog/knowledge-graph-vs-vector-database-for-rag>)
- [GraphRAG: Unlocking LLM Discovery on Narrative Private Data — Microsoft Research Blog](<https://www.microsoft.com/en-us/research/blog/graphrag-unlocking-llm-discovery-on-narrative-private-data/>)
- [Project GraphRAG — Microsoft Research](<https://www.microsoft.com/en-us/research/project/graphrag/>)
- [A Semantic Approach to Mapping the Provenance Ontology to Basic Formal Ontology — Scientific Data](<https://www.nature.com/articles/s41597-025-04580-1>)
- [Notion vs Obsidian - minimalism as user preference (NotionApps)](<https://www.notionapps.com/blog/notion-vs-obsidian-knowledge-productivity-2025>)
- [The Semantic Web: 20 Years and a Handful of Enterprise Knowledge Graphs Later — Ontotext](<https://www.ontotext.com/blog/the-semantic-web-20-years-later/>)
- [Notion vs Obsidian vs Roam Research 2025: Best Note-Taking App for Productivity](<https://www.primeproductiv4.com/blog-articles/notion-vs-obsidian-vs-roam-research-productivity-comparison>)
- [History of Obsidian: Second Brain to AI Knowledge OS — Taskade Blog](<https://www.taskade.com/blog/obsidian-history>)
- [AI-Driven Knowledge Management System Market Report (The Business Research Company) - the '$7.71B 2025 / 47.2%' figure traces here, not to GoSearch; cross-firm AI-KM sizing varies widely and the finding's two growth rates do not reconcile](<https://www.thebusinessresearchcompany.com/report/ai-driven-knowledge-management-system-global-market-report>)
- [Neo4j Software Pricing & Plans 2026 (Vendr)](<https://www.vendr.com/marketplace/neo4j>)
- [SKOS Simple Knowledge Organization System - W3C Home Page](<https://www.w3.org/2004/02/skos/>)
- [RDF & SPARQL Working Group Charter — W3C (April 2025)](<https://www.w3.org/2025/04/rdf-star-wg-charter.html>)
- [JSON-LD 1.1 — W3C Recommendation](<https://www.w3.org/TR/json-ld11/>)
- [PROV-O: The PROV Ontology - W3C Recommendation](<https://www.w3.org/TR/prov-o/>)
- [PROV-Overview — W3C](<https://www.w3.org/TR/prov-overview/>)
- [SKOS Simple Knowledge Organization System Primer - W3C Recommendation](<https://www.w3.org/TR/skos-primer/>)
- [SKOS Simple Knowledge Organization System Reference — W3C](<https://www.w3.org/TR/skos-reference/>)
- [Ontologies and Knowledge Graphs in Industry Community Group — W3C](<https://www.w3.org/community/oki/>)
- [YAML-LD — W3C CG Final Report, December 2023](<https://www.w3.org/community/reports/json-ld/CG-FINAL-yaml-ld-20231206/>)
- [The PROV-JSONLD Serialization - W3C Member Submission 2024](<https://www.w3.org/submissions/2024/SUBM-prov-jsonld-20240825/>)
- [Introducing MIF: Memory Interchange Format — zircote.com (February 2026)](<https://zircote.com/blog/2026/02/introducing-mif-memory-interchange-format/>)
- [AI Agent Memory Architectures: From Context Windows to Persistent Knowledge — Zylos Research](<https://zylos.ai/research/2026-04-05-ai-agent-memory-architectures-persistent-knowledge/>)
