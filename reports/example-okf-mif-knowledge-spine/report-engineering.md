---
slug: reports/example-okf-mif-knowledge-spine/report-engineering
version: 1
...
```mermaid
flowchart TD
  A[OKF bundle authoring: markdown plus YAML frontmatter] --> B[Git distribution: permissive, tolerate broken links]
  B --> C[Ingestion enrichment pass]
  C --> D[Classify untyped prose links into typed MIF relationships]
  C --> E[Resolve ontology and entity typing]
  C --> F[Add provenance block and bi-temporal fields]
  D --> G{Fail-closed MIF validation}
  E --> G
  F --> G
  G -->|valid| H[Typed MIF graph companions beside OKF markdown]
  G -->|invalid| I[Reject and return to authoring]
  H --> J[MIF to OKF export: types degrade to untyped links plus prose]
  J --> A
```
