---
slug: reports/example-okf-mif-knowledge-spine/report-trend-modeling
version: 1
...
```mermaid
xychart-beta
    title "KM software market, USD billions (CAGR-interpolated projection)"
    x-axis [2025, 2028, 2031, 2034]
    y-axis "USD billions" 0 --> 90
    bar [23, 35, 53, 81]
    line [23, 35, 53, 81]
```
...
```mermaid
quadrantChart
    title Scenario matrix: OKF distribution vs demand for enrichment
    x-axis Low OKF distribution --> High OKF distribution
    y-axis Low enrichment demand --> High enrichment demand
    quadrant-1 S1 Layered spine wins
    quadrant-2 S4 Incumbents hold or MIF goes direct
    quadrant-3 S3b Both stall
    quadrant-4 S2 Minimalism as ceiling
    S1 favorable: [0.78, 0.80]
    S2 unfavorable: [0.80, 0.22]
    S3 absorption: [0.62, 0.55]
    S4 fragmentation: [0.25, 0.70]
```

```mermaid
stateDiagram-v2
    [*] --> Baseline
    Baseline: Baseline mid-2026 (OKF v0.1 plus MIF v1.0.0)
    Baseline --> OKFAdopted: Google backing compounds distribution
    Baseline --> OKFStalls: OKF distribution fails to compound
    OKFAdopted --> S1: buyers hit OKF minimalism ceiling, adopt MIF layer
    OKFAdopted --> S2: minimalism is the buyer preference
    OKFAdopted --> S3: OKF or a standards body absorbs provenance and typed links
    OKFStalls --> S4: agent-memory formats stay fragmented
    S1: S1 Layered spine wins (favorable)
    S2: S2 Minimalism as ceiling (unfavorable)
    S3: S3 Convergence or absorption
    S4: S4 Fragmentation or stall
    S1 --> [*]
    S2 --> [*]
    S3 --> [*]
    S4 --> [*]
