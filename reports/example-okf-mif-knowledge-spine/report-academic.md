---
slug: reports/example-okf-mif-knowledge-spine/report-academic
version: 1
...
```mermaid
flowchart TB
  subgraph OKF["OKF v0.1 — authoring and distribution surface"]
    A1["Markdown files + YAML frontmatter"]
    A2["Required field: type (producer-defined)"]
    A3["Untyped markdown links"]
    A4["timestamp (last-modified only)"]
    A5["log.md and #Citations (prose convention)"]
  end
  subgraph MIF["MIF v1.0.0 — modeling, provenance and temporal spine"]
    M1["entity + ontology (formal typing)"]
    M2["relationships[] (typed, directed, strength 0-1)"]
    M3["provenance block (PROV-O compatible)"]
    M4["temporal block (bi-temporal + decay)"]
    M5["citations[] (typed roles)"]
  end
  A2 -->|formalized by| M1
  A3 -->|enriched into| M2
  A5 -->|structured as| M3
  A4 -->|extended to| M4
  A5 -->|structured as| M5
  OKF -->|extended frontmatter / JSON companions| MIF
```
...
```mermaid
timeline
  title Convergent milestones of the OKF + MIF knowledge-spine trajectory
  Feb 2026 : MIF introduced publicly : interoperability spec, three conformance levels
  Apr 2026 : Karpathy LLM-Wiki gist : 5,000+ stars and forks, wave of implementations
  Jun 2026 : OKF v0.1 published by Google Cloud (5,440+ stars) : MIF stabilized at v1.0.0 (Released)
  2028 (Gartner) : 50%+ of AI agent systems projected to use context graphs
```
